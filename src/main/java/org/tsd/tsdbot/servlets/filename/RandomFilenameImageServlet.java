...
@Singleton
public class RandomFilenameImageServlet extends HttpServlet {

    private static final Logger logger = LoggerFactory.getLogger(RandomFilenameImageServlet.class);
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        logger.info("Fetching random filename from path: {}", path);
        String filename = null;
        try{
            filename = path.split("/")[1];
            logger.info("Parsed random filename {}", filename);
            byte[] data = filenameLibrary.getRandomFilename(filename);
