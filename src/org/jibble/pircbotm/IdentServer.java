...
public class IdentServer extends Thread {
    ...
    private final static Logger LOGGER = Logger.getLogger(IdentServer.class.getName());
    ...
    IdentServer(PircBot bot, String login) {
        ...
        try {
            ...
            LOGGER.severe("*** Could not start the ident server on port 113.");
            return;
        }

        LOGGER.finest("*** Ident server running on port 113 for the next 60 seconds...");
    ...
    public void run() {
        try {
            ...
            if (line != null) {
                LOGGER.finest("*** Ident request received: " + line);
                line = line + " : USERID : UNIX : " + _login;
                writer.write(line + "\r\n");
                writer.flush();
                LOGGER.finest("*** Ident reply sent: " + line);
                writer.close();
            }
        }
        catch (Exception e) {
        ...
        LOGGER.finest("*** The Ident server has been shut down.");
    }
