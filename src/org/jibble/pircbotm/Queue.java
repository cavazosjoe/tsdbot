...
public class Queue {
    ...
    /**
     * Puts a String for the given target to the end of the queue.
     * 
     * @param target
     *            message target, used to implement some kind of fariness
     * @param message
     */
    public void add(String target, String message) {
        synchronized (_queue) {
            _queue.add(new IRCMsg(target, message));
            _queue.notify();
        }
    }
    ...
    public void add(String message) {
        synchronized (_queue) {
            _queue.add(new IRCMsg("p:i:r:c", message)); //$NON-NLS-1$
            _queue.notify();
        }
    ...
    private java.util.Queue<IRCMsg> _queue = new MsgQueue();
